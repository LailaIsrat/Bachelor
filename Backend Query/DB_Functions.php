...
$stmt->bind_param("sssssssssssssssss",$FirstName,$LastName,$Email,$Local_Guardians_Name,$Guardians_Mobile_No,$Birth_Certificate_No,$National_ID_Card_No,$University_Name,$University_ID,$Company_Name,$Employ_ID,$MobileNumber,$Password,$Area,$House,$Flat,$Road);
$result=$stmt->execute();
$stmt->close();
...
$stmt->bind_param("sssssddssssssssssd",$FirstName,$LastName,$Gender,$Email,$Local_Guardians_Name,$Guardians_Mobile_No,$National_ID_Card_No,$University_Name,$University_ID,$Company_Name,$Employ_ID,$MobileNumber,$Password,$Area,$House,$Flat,$ID);
$result=$stmt->execute();
$stmt->close();
...
$stmt->close();
 ...
 if($stmt=$this->conn->prepare("SELECT *FROM b_order WHERE (User_ID = b_order.User_ID AND Total_Meal=(SELECT max(Total_Meal) FROM b_order WHERE User_ID = ?))"))
 {
...
public function getOrderByPaymentStatus($Total_Meal,$Total_cost,$Total_Lunch,$Total_Dinner,$Meal_Rate,$To_Pay,$Paid_Amount,$User_ID){
   // echo"$Total_Meal";
$stmt=$this->conn->prepare("SELECT * FROM b_order WHERE (Order_ID = (SELECT max(Order_ID) FROM b_order  WHERE User_ID = ?))");
...
public function getOrderByPaymentStatusbefore($User_ID){
